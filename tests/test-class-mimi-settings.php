<?php
class Test_Mad_Mimi_Settings extends WP_UnitTestCase {
	...
	public function test_construct() {
		$instance = new Mad_Mimi_Settings();
	...
	public function test_action_admin_menu() {
		...
		$this->assertEquals( 'mimi-settings', $instance->slug );
	}
	...
	public function test_page_load() {
		...
		$this->assertTrue( wp_style_is( 'mimi-admin', 'registered' ) );
		...
		$this->assertEquals( $sample_data, get_transient( 'mimi-user_name-lists' ) );
		...
		$this->assertFalse( get_transient( 'mimi-user_name-lists' ) );
		...
		set_transient( 'mimi-user_name-lists', $sample_data );
		set_transient( 'mimi-form-the_id', $sample_data_2 );
		...
		$this->assertEquals( $sample_response, json_encode( get_transient( 'mimi-user_name-lists' ) ) );
		$this->assertFalse( get_transient( 'mimi-form-the_id' ) );
		$this->assertNotEmpty( $errors );
		$this->assertEquals( 'mimi-reset', $errors[0]['code'] );
		...
		$instance->page_load();
		$meta = get_user_meta( 12345, 'mimi-dismiss' );
	...
	public function test_setup_help_tabs() {
		...
		$current_screen = WP_Screen::get( 'test_mimi' );

		$instance = new Mad_Mimi_Settings();
		$instance->setup_help_tabs();

		$tabs = $current_screen->get_help_tabs();
		$this->assertArrayHasKey( 'mimi-overview', $tabs );
		$this->assertContains( 'Mad Mimi', $current_screen->get_help_sidebar() );
	}
	...
	public function test_register_settings() {
		...
		$this->assertArrayHasKey( 'mimi-options', $new_whitelist_options );
		$this->assertEquals( $instance->slug, $new_whitelist_options['mimi-options'][0] );
	...
	public function test_display_settings_page() {
		...
		ob_end_clean();
		$this->assertContains( "<input type='hidden' name='option_page' value='mimi-options' />", $actual_output );
		$this->assertContains( '<input type="text" name="mimi-settings[username]"', $actual_output );
		$this->assertContains( '<label for="mimi-settings-username">Mad Mimi Sign Up Forms Username</label>', $actual_output );
		$this->assertContains( '<label for="mimi-settings-api-key">Mad Mimi Sign Up Forms API Key</label>', $actual_output );
		$this->assertContains( '<input type="text" name="mimi-settings[api-key]"', $actual_output );
		$this->assertContains( '<input type="checkbox" name="mimi-settings[display_powered_by]" id="mimi-settings[display_powered_by]" value="1"  />', $actual_output );
	...
	public function test_display_settings_page_forms() {
		...
		update_option( 'mimi-settings', array(
			'username' => 'tester',
			'api-key'  => '12345',
		) );
		$instance = new Mad_Mimi_Settings();
		...
		delete_transient( 'mimi-tester-account' );
		delete_transient( 'mimi-tester-lists' );
		delete_transient( 'mimi-form-54321' );
	}
